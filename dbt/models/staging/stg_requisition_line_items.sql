...
select
  ...
  JSONExtract(after, 'totalreceivedquantity', 'Nullable(Int64)')    as total_received_quantity,
  JSONExtract(after, 'totalconsumedquantity', 'Nullable(Int64)')    as total_consumed_quantity,
  JSONExtractInt(after, 'totallossesandadjustments')                as total_losses_and_adjustments,
  JSONExtract(after, 'stockonhand', 'Nullable(Int64)')              as stock_on_hand,
  JSONExtractInt(after, 'totalstockoutdays')                        as total_stockout_days,
  JSONExtract(after, 'averageconsumption', 'Nullable(Int64)')       as average_consumption,
  JSONExtract(after, 'adjustedconsumption', 'Nullable(Int64)')      as adjusted_consumption,
  ...
  JSONExtract(after, 'totalcost', 'Nullable(Float64)')              as total_cost,
