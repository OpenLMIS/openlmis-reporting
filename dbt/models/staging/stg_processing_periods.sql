{{
  config(
    materialized='view'
  )
}}

-- Current-state reconstruction for referencedata.processing_periods.

with ranked as (
  select
    *,
    row_number() over (
      partition by coalesce(
        nullIf(JSONExtractString(after,  'id'), ''),
        nullIf(JSONExtractString(before, 'id'), '')
      )
      order by ts_ms desc, _ingested_at desc
    ) as _rn
  from raw.events_openlmis_referencedata_processing_periods
  where coalesce(
        nullIf(JSONExtractString(after,  'id'), ''),
        nullIf(JSONExtractString(before, 'id'), '')
      ) != ''
)

select
  toUUID(JSONExtractString(after, 'id'))                       as id,
  JSONExtractString(after, 'name')                             as name,
  -- Debezium encodes DATE columns as epoch days (int); the bootstrap path
  -- (row_to_json) emits ISO strings. Accept both.
  coalesce(
    toDateOrNull(JSONExtractString(after, 'startdate')),
    toDate(toDate('1970-01-01') + JSONExtractInt(after, 'startdate'))
  )                                                            as start_date,
  coalesce(
    toDateOrNull(JSONExtractString(after, 'enddate')),
    toDate(toDate('1970-01-01') + JSONExtractInt(after, 'enddate'))
  )                                                            as end_date,
  toUUIDOrNull(JSONExtractString(after, 'processingscheduleid'))       as processing_schedule_id,
  JSONExtractString(after, 'description')                      as description
from ranked
where _rn = 1
  and op != 'd'
